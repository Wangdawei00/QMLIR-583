...
prev_quantum_opt = sh.Command("/artifact/QMLIR/build/bin/quantum-opt")
class PrevQSSAOpt(Opt):
    def load(self, filename):
        """
        ...
        self.outfile = filename.replace('circuit_qasm', 'circuit_mlir2').replace('.qasm', '.qssa.opt.mlir')
        self.taskname = os.path.basename(filename).replace('.qasm', '')

        self.data = None

    def opt(self):
        prev_quantum_opt(self.filename,
                    '--inline',
                    '--cse',
                    '--symbol-dce',
                    '--qssa-apply-rewrites',
...
quantum_opt = sh.Command("/artifact/QMLIR/opt_build/bin/quantum-opt")
class QSSAOpt(Opt):
    def load(self, filename):
        """
        loads circuit_mlir/<file>.qssa.mlir
...
class BenchmarkOne:
    def __init__(self, filename):
        ...
        self.opts['qssa_opt'] = QSSAOpt(filename)
        self.opts['qssa'] = PrevQSSAOpt(filename)
